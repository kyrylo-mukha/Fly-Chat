...
**Layout spec.** The input bar is a floating, Telegram-style composer with no background plate: a circular `FCLGlassIconButton` attach button, a rounded `FCLGlassContainer` text capsule (built with `interactive: true`), and a circular `FCLGlassIconButton` send button tinted with the outgoing-bubble color (prominent glass on iOS 26). The text field is a native `TextField(_:text:axis:)` that auto-grows vertically up to a configured maximum row count. The composer floats over the timeline via `.overlay(alignment: .bottom)`; the bottom-anchored `List` is full-bleed (`scrollContentBackground(.hidden)`) and reserves a clear spacer equal to the measured composer height so the newest message rests above the bar while older cells scroll beneath the glass. Hosts can replace the whole bar via `@ViewBuilder customInputBar`.
...
**Layout spec.** A custom `AVCaptureSession`-driven screen with live preview filling the canvas and three glass-based chrome rows overlaid. The top bar (`FCLCameraTopBar`) floats three independent capsule elements at 16 pt insets with `space-between` alignment: a close `FCLGlassIconButton` (leading), a flash mode `FCLGlassChip` pill showing a glyph + label (center), and an optional overflow `FCLGlassIconButton` (trailing; an invisible 44 pt placeholder maintains center alignment when omitted). Below the live preview, three bottom-chrome rows are stacked with 18 pt gaps and 44 pt bottom padding: a zoom preset ring (`FCLCameraZoomPresetRing`) using `FCLGlassChip` segments with white active fill and numeric-only labels on deselected chips; a centered shutter row (`FCLCameraShutterRow`) in a `1fr | auto | 1fr` HStack — leading Done chip (`FCLGlassChip` with badge, visible at count ≥ 2), 78 pt shutter button, trailing dashed reserved slot; and a photo/video mode switcher (`FCLCameraModeSwitcherRow`) — flip `FCLGlassIconButton` and a segmented chip pair with white active fill. A focus reticle animates on tap; a record-timer pill appears below the top bar while recording.
...
**Layout spec.** A full-screen pager that aspect-fits each asset. A parallax thumbnail strip on an `FCLGlassContainer` anchors its **top edge** `88pt` above the bottom safe area, scrolls at half-rate relative to the pager, and emphasizes the active index (center thumb at full opacity and scale; side thumbs at ~0.5 opacity). The cover is transparent so the chat timeline remains visible behind, which matters for visibility-aware dismiss animations. A close button appears top-left on an `FCLGlassIconButton`.

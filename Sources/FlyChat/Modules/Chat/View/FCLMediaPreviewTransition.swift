...
///
/// The chat screen (or any host that presents ``FCLMediaPreviewView``) adopts this protocol
/// so the preview can query the source frame for zoom-in / zoom-out style dismissal.
///
...
/// reference-type relay instead. The attachment grid reports the window-space frames
/// of visible cells into the relay's ``frames`` dictionary (keyed by
/// `attachment.id.uuidString`). The preview view reads those frames on dismiss to
/// animate content back into the originating cell; if the key is missing the preview
/// collapses to a zero-size point at the screen center.
@MainActor
